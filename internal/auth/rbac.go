...
type RBACManager interface {
	...
	HasPermissionWithContext(ctx context.Context, user *UserIdentity, resource, action, target string) bool
...
func (r *InMemoryRBAC) SetTracer(tracer trace.Tracer) {
	r.tracer = tracer
}
...
func (r *InMemoryRBAC) CreateRole(role *Role) error {
	...
	r.metrics.RecordRoleOperation("create", time.Since(start))
	return nil
}
...
func (r *InMemoryRBAC) HasPermission(user *UserIdentity, resource, action, target string) bool {
	return r.HasPermissionWithContext(context.Background(), user, resource, action, target)
}

// HasPermissionWithContext checks if user has permission for resource/action/target with tracing context
func (r *InMemoryRBAC) HasPermissionWithContext(ctx context.Context, user *UserIdentity, resource, action, target string) bool {
	// Start distributed tracing span
	var span trace.Span
	if r.tracer != nil {
		ctx, span = telemetry.StartSpan(ctx, r.tracer, "rbac.has_permission",
			telemetry.NewSpanAttributeBuilder().
				Component("rbac").
				Operation("has_permission").
				String("rbac.resource", resource).
				String("rbac.action", action).
				String("rbac.target", target).
				Build()...)
		defer span.End()
	}
	...
	if user == nil {
		r.metrics.RecordPermissionCheck(false, time.Since(start))
		if span != nil {
			telemetry.AddEvent(span, "permission.denied.no_user")
			telemetry.SetSpanAttributes(span, telemetry.NewSpanAttributeBuilder().
				Bool("rbac.granted", false).
				String("rbac.deny_reason", "no_user").
				Build()...)
	...
	if span != nil {
		telemetry.SetSpanAttributes(span, telemetry.NewSpanAttributeBuilder().
			...
			String("rbac.user_roles", fmt.Sprintf("%v", user.Roles)).
			Build()...)
	}
	...
	policyResult := r.checkPolicies(user, resource, action, target)
	if policyResult == "deny" {
		r.metrics.RecordPermissionCheck(false, time.Since(start))
		if span != nil {
			telemetry.AddEvent(span, "permission.denied.policy")
			telemetry.SetSpanAttributes(span, telemetry.NewSpanAttributeBuilder().
				Bool("rbac.granted", false).
				String("rbac.deny_reason", "policy_deny").
				Build()...)
			telemetry.RecordSuccess(span)
		}
		return false
	...
	if span != nil {
		telemetry.SetSpanAttributes(span, telemetry.NewSpanAttributeBuilder().
			String("rbac.effective_roles", fmt.Sprintf("%v", allRoles)).
			Int("rbac.role_count", len(allRoles)).
			Build()...)
	}
	...
	for _, roleName := range allRoles {
		...
		if r.checkRolePermissions(role, resource, action, target) {
			r.metrics.RecordPermissionCheck(true, time.Since(start))
			if span != nil {
				telemetry.AddEvent(span, "permission.granted.role")
				telemetry.SetSpanAttributes(span, telemetry.NewSpanAttributeBuilder().
					Bool("rbac.granted", true).
					String("rbac.granted_by_role", roleName).
					Build()...)
				telemetry.RecordSuccess(span)
			}
			return true
		...
		for _, inheritedRole := range role.Inherits {
			if inherited, exists := r.roles[inheritedRole]; exists {
				if r.checkRolePermissions(inherited, resource, action, target) {
					r.metrics.RecordPermissionCheck(true, time.Since(start))
					if span != nil {
						telemetry.AddEvent(span, "permission.granted.inherited_role")
						telemetry.SetSpanAttributes(span, telemetry.NewSpanAttributeBuilder().
							Bool("rbac.granted", true).
							String("rbac.granted_by_role", roleName).
							String("rbac.granted_by_inherited_role", inheritedRole).
							Build()...)
						telemetry.RecordSuccess(span)
					}
					return true
	...
	r.metrics.RecordPermissionCheck(false, time.Since(start))
	if span != nil {
		telemetry.AddEvent(span, "permission.denied.no_matching_role")
		telemetry.SetSpanAttributes(span, telemetry.NewSpanAttributeBuilder().
			Bool("rbac.granted", false).
			String("rbac.deny_reason", "no_matching_role").
			Build()...)
		telemetry.RecordSuccess(span)
	}
	return false
...
func (r *InMemoryRBAC) GetMetrics() map[string]interface{} {
